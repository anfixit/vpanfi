...
def trial_ready_letter(
    *,
    ...
    """Письмо о выданных пробных днях.

    Регистрация выдавала доступ и молчала: уведомление уходило
    владельцу сервиса, а человеку не уходило ничего. Он должен был сам
    догадаться вернуться в кабинет за ссылкой, и не догадывался.
    С 13.08 по 03.09.2026 так не подключились четверо из четверых.

    Поэтому ссылка лежит прямо в письме, как и в письме о покупке.
    """
    until = expires_at.strftime("%d.%m.%Y")
    text = f"""Добро пожаловать в VPaNfi!

Вам открыты {days} дней бесплатно, до {until}. Платить пока ничего
не нужно, и само собой ничего не спишется.

Вот ваша ссылка, скопируйте её целиком:

{subscription_url}

Что с ней делать: установите приложение и вставьте ссылку в него
через «Вставить из буфера обмена». Сама по себе ссылка ничего
не включает.
...
<body style="margin:0;padding:24px;background:#f6f5f2;
  ...
  <div style="max-width:560px;margin:0 auto;background:#ffffff;
              border-radius:16px;padding:28px;">
    <h1 style="font-size:22px;margin:0 0 16px;">Добро пожаловать
      в VPaNfi</h1>

    <p style="margin:0 0 16px;">Вам открыты <strong>{days} дней
      бесплатно</strong>, до <strong>{until}</strong>. Платить пока
      ничего не нужно, и само собой ничего не спишется.</p>
    ...
    <p style="margin:0 0 16px;">Установите приложение и вставьте ссылку
      в него через «Вставить из буфера обмена». Сама по себе ссылка
      ничего не включает.</p>
    ...
    <p style="margin:0 0 8px;color:#6b6b6b;font-size:14px;">Если что-то
      не получается, напишите мне:</p>
    <p style="margin:0;color:#6b6b6b;font-size:14px;line-height:1.8;">
      <a href="mailto:{safe_mail}"
         style="color:#5b53d6;">{escape(support_email)}</a>
      , почта, работает без VPN<br>
    ...
    return Letter(
        subject=f"Ваши {days} дней VPaNfi готовы",
        text=text,
        html=html,
    )
